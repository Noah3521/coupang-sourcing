"""MCP server exposing coupang-sourcing as typed tools for OpenClaw / Hermes agents.

Run as a stdio MCP server and register it in the agent's config (see docs/OPENCLAW.md):

    mcp_servers:
      coupang:
        command: "/path/to/.venv/bin/coupang-sourcing-mcp"

The agent then gets typed tools (mcp_coupang_find_products, ...). Everything is reused from
the CLI collectors/storage, so the same SQLite DB accumulates sourcing data over time.
    ...
    try:
        ...
        for pid, variants in list(index.items())[: max(1, limit)]:
            ...
            record = product_mod.collect_product(
                client, ids, store_info, cfg, variants=variants
            )
            storage.save_record(cfg.db_path, record)
            collected += 1
